...
export default function WalletSendDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={() => setOpen(false)}>
      <DialogContent className="w-full max-w-md bg-[#0d0d0d] rounded-xl text-white p-5">
        <DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSend)} className="space-y-6">
              ...
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Button
                      variant="outline"
                      className="w-full h-14 justify-start text-left bg-black border-neutral-700"
                      ...
                      {selectedToken ? (
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium text-white text-sm">
                            {selectedToken.metadata?.symbol || "Unknown"}
                          </span>
                        </div>
                      ) : (
                        t("tx.selectToken")
                        ...
                        onClick={() =>
                          form.setValue(
                            ...
                            selectedToken?.balance
                              ? (Number(selectedToken.balance) / 2).toFixed(6)
                              : "",
                          )
                        }
                      >
            ...
            <div className="flex gap-2 pt-2">
              ...
              <Button
                type="submit"
                className="flex-1 bg-white text-black hover:bg-neutral-300"
              >
                {t("wallet.send")}
              </Button>
            </div>
      ...
      <TokenDialog open={selectOpen} onOpenChange={setSelectOpen}>
        <TokenDialogContent className="w-full max-w-md bg-[#111] text-white">
          <Input
            placeholder={t("token.search")}
            value={tokenSearch}
            onChange={(e) => setTokenSearch(e.target.value)}
            className="mb-3 bg-neutral-900 text-white border border-neutral-700"
          />
          <ScrollArea className="max-h-72">
            {filteredTokens.map((token: any) => (
              <button
                ...
                className="flex items-center gap-3 w-full px-3 py-2 rounded-md hover:bg-neutral-800 transition"
              >
                <Avatar className="h-6 w-6">
